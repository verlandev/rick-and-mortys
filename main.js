...
// Creamos el botón de búsqueda
            ...
            const searchButton$$ = document.querySelector('.search__button')
                ...
                // Primero buscará y luego filtrará
                        const search = () => {
                            const charactersFiltered = characters.filter((character) => { // character es un PUTO PARAMETRO Y SE LO PONES TÚ SIN PUTO MIEDO. Como si lo llamas "meCagoEnSusMuertos" que te va a hacer caso
            ...
            const selectOptions$$ = document.querySelector('#statusFilter');
                        ...
                        const statusFilter = () => {
                            ...
                            const characterByStatus = characters.filter((character) => {
                                ...
                                }else if(character.status.includes('Dead')){ // No sé por qué este no funciona
                                        return character.status
                                        .includes(selectOptions$$.value)
                                    }
                                })
...
const fetchData = async () => { //Espero a que llegue el camarero para pedir (los 5 minutos de rigor)
